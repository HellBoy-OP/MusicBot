...
async def play(_, message: Message):
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        position = queues.add(message.chat.id, file_path)
        queues.add(message.chat.id, file_path)
        queues.add(message.chat.id, file_path)
        await res.edit_text(f"**{Bn} :** #️⃣ Will play this 3 times after {position} songs")
    else:
        await res.edit_text(f"**{Bn} :** ▶️ Playing This song 3 Times...")
        callsmusic.pytgcalls.join_group_call(message.chat.id, file_path, 48000, callsmusic.pytgcalls.get_cache_peer())
        queues.add(message.chat.id, file_path)
        queues.add(message.chat.id, file_path)
